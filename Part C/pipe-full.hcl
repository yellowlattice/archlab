...
####################################################################
#   Leader's name : »ŒΩ‡ Renjie                                    #
#   STUDENT ID : 516021910425                                      #
####################################################################

###################         Analysis         #######################
#     We analyze the function in ncopy.c, finding that there are   #
#  mainly there operations slowing down the function :             #
#  1. irmovl $1, %edi                                              #
#	  addl %edi, %eax	                                   #
#  2. mrmovl(%ebx), %esi	                                   #
...
###################         Solution         #######################
#     In full.hcl, we implement the "iaddl" to solve the first     #
#  problem. Similar to seq-full.hcl, we just need to specify some  #
#  registers.                                                      #
#                          #############                           #
# Fetch         f_icode:ifun <- M[PC]                              #
#               rA : rB <- M[PC + 1]                               #
#               rC <- M[PC + 2]                                    #
#               valP <- PC + 1                                     #
#                          #############                           #
# Decode        D_valB <- R[D_rB]                                  #
#                                                                  #
#                          #############                           #
...
# Memory        NULL                                               #
#                          #############                           #
# Write Back    R[D_rB] <- E_valE                                  #
#                          #############                           #
# PC update     PC <- F_predPC                                     #
####################################################################
...
IIRMOVL, IRMMOVL, IMRMOVL ,IIADDL };                               # iaddl contains register

# Does fetched instruction require a constant word ?
bool need_valC =
f_icode in{ IIRMOVL, IRMMOVL, IMRMOVL, IJXX, ICALL ,IIADDL };      # iaddl contains a constant word
...
int d_srcB = [
	D_icode in{ IOPL, IRMMOVL, IMRMOVL ,IIADDL }:D_rB;             # iaddl uses R[rB]
...
int d_dstE = [
	D_icode in{ IRRMOVL, IIRMOVL, IOPL, IIADDL }:D_rB;             # the result will be writen back to R[rB]
	D_icode in{ IPUSHL, IPOPL, ICALL, IRET } : RESP;
...
int aluA = [
	...
	E_icode in{ IIRMOVL, IRMMOVL, IMRMOVL,IIADDL } : E_valC;       # R[rB] add valC , meaning E_valC
...
int aluB = [
	E_icode in{ IRMMOVL, IMRMOVL, IOPL, ICALL,                     # R[rB] add valC , meaning E_valC
...
bool set_cc = E_icode in{ IOPL,IIADDL } &&                         # iaddl update condition codes
